...
export default function PengawasanPemasanganPage() {
  ...
  const [search, setSearch] = useFilterPersist('pengawasan-pemasangan-search', '');
        ...
        <Card>
          <CardContent>
            {loading ? (
              ...
              <EmptyState
                title='Belum ada data pengawasan pemasangan'
                description={search ? 'Tidak ada hasil yang cocok dengan pencarian Anda' : 'Belum ada work order pengawasan pemasangan yang disubmit oleh teknisi'}
              />
            ) : (
              <>
                  ...
                  <Table size='small' sx={{ minWidth: 600 }}>
                    ...
                    <TableBody>
                      {paginated.map((wo, idx) => {
                        ...
                        return (
                          <TableRow key={wo.id} hover onClick={() => router.push(`/operations/pengawasan-pemasangan/${wo.id}`)} sx={{ cursor: 'pointer' }}>
