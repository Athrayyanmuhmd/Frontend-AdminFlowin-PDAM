...
export default function PemasanganPage() {
  ...
  const [search, setSearch] = useFilterPersist('pemasangan-search', '');
  ...
  return (
    <AdminLayout title='Data Pemasangan'>
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 1 }}>
          <Box>
            <Typography variant='h5' fontWeight={700}>Data Pemasangan</Typography>
            <Typography variant='body2' color='text.secondary'>
              Work order pemasangan yang telah disubmit oleh teknisi
            </Typography>
          </Box>
        ...
        <Card>
          <CardContent>
            {loading ? (
              ...
              <EmptyState
                title='Belum ada data pemasangan'
                description={search ? 'Tidak ada hasil yang cocok dengan pencarian Anda' : 'Belum ada work order pemasangan yang disubmit oleh teknisi'}
              />
            ) : (
              <>
                  ...
                  <Table size='small' sx={{ minWidth: 600 }}>
                    ...
                    <TableBody>
                      {paginated.map((wo, idx) => {
                        ...
                        return (
                          <TableRow key={wo.id} hover onClick={() => router.push(`/operations/pemasangan/${wo.id}`)} sx={{ cursor: 'pointer' }}>
                            ...
                            <TableCell align='center'>
                              <Tooltip title='Lihat Detail'>
                                <IconButton size='small' color='primary' onClick={() => router.push(`/operations/pemasangan/${wo.id}`)}>
