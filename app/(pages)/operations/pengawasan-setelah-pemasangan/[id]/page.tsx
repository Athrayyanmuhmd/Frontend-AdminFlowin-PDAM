...
export default async function PengawasanSetelahPemasanganDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 3 }}>
        <Button component={Link} href='/operations/pengawasan-setelah-pemasangan' startIcon={<ArrowBack />} variant='text' size='small'>
          Kembali
        </Button>
        <Typography variant='h6' fontWeight={700}>Detail Pengawasan Setelah Pemasangan</Typography>
      </Box>
      ...
      <Grid container spacing={3}>
        ...
        {/* Right: Progres Pengawasan Setelah Pemasangan */}
        <Grid item xs={12} md={7}>
          <Card>
            <CardContent>
              <Typography variant='subtitle1' fontWeight={700} sx={{ mb: 2 }}>Data Pengawasan Setelah Pemasangan</Typography>
              <Divider sx={{ mb: 2 }} />
              {!progres ? (
                <Typography color='text.secondary'>Belum ada data progres pengawasan setelah pemasangan</Typography>
              ) : (
                <>
